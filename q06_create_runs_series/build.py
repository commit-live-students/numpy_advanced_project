...
def create_delivery_series():
	del_series = pd.Series(data=npArray_filter[:, 12])
	return del_series
	
	
def create_runs_series(strMatchCode):
	delRuns_series = pd.Series(data=npArray_filter[:, 16], index = npArray_filter[:, 11])
	return delRuns_series
